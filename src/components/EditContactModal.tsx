...
export default function EditContactModal({ contact, onClose, onUpdate, onToast, contacts }: EditContactModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!isValid) {
      onToast(error || "Invalid Indian phone format.", "error");
      return;
    }
    ...
    >
      ...
      >
        <div className="flex justify-between items-center">
          <h3 className="text-lg font-bold text-slate-900 dark:text-white">Modify Contact</h3>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 mt-4">
          <div>
            ...
            <div className="relative">
              ...
              <input id="edit-contact-name-textbox" type="text" required placeholder="e.g. Pastor Mark"
                value={name} onChange={(e) => setName(e.target.value)}
                className="w-full pl-9 pr-3 py-2.5 border border-slate-200 dark:border-slate-600 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 bg-slate-50/20 dark:bg-slate-700/50 dark:text-white dark:placeholder-slate-400" />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input id="edit-contact-phone-textbox" type="text" required placeholder="+91 XXXXX XXXXX"
                value={phone} onChange={(e) => setPhone(e.target.value)}
                className="w-full pl-9 pr-3 py-2.5 border border-slate-200 dark:border-slate-600 rounded-lg text-sm font-mono focus:outline-none focus:ring-2 focus:ring-emerald-500 bg-slate-50/20 dark:bg-slate-700/50 dark:text-white dark:placeholder-slate-400" />
            </div>
          </div>
          ...
          <div className="flex gap-2 justify-end mt-2">
            ...
            <button id="edit-contact-submit-btn" type="submit" className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-semibold rounded-lg transition-all cursor-pointer shadow-sm active:scale-95">
              Apply Edits
            </button>
          </div>
        </form>
